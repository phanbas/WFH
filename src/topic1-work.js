...
export default function Topic1Work(){
    const [data, setData] = useState([])
  useEffect(()=>{
    fetch("https://phanbas-strapi.herokuapp.com/api/works")
    .then(res =>res.json())
    ...
    return (
        ...
        <div className='topic-text'>
            {data.map((val)=>
                    val.id===1 && <p className='topic-text'>{val.attributes.name}</p>
                )}
        </div>
        ...
        <br/>
            {data.map((val)=>
                val.id===1 && <h2 className='desc'>&emsp;&emsp;{val.attributes.desc}</h2>
            )}
        <br/>
        </Card>;
        ...
        <div style={{ width: '80%', padding:20, textAlign:'center',margin:'auto',marginTop:50 }}>
            ...
            {data.map((val)=>
                val.id===2 && <Card.Title><h3 className='name'>{val.attributes.name}</h3></Card.Title>
